...
This is a reference application demonstrating the three-layer architecture (Angular micro front ends → Express BFF → Express domain APIs) documented in [suite-architecture-standards.md](../suite-architecture-standards.md). It is intentionally minimal. Its purpose is to demonstrate the architecture correctly, not to build production payment infrastructure. Every architectural principle from A1 through E12 is implemented and visible in the running app. The **⚡ Arch** panel narrates architectural decisions in real-time as you use the application.
...
**Two surfaces → Two BFFs → Same domain APIs.** See [PARTNER-BFF-A2-DEMO.md](PARTNER-BFF-A2-DEMO.md) for side-by-side comparisons showing different auth, response shapes, and payment patterns for the same underlying bill data.
...
See [TEST-RESULTS.md](TEST-RESULTS.md) for full test documentation.
...
| [apps/web-bff/src/middleware/correlation.ts](apps/web-bff/src/middleware/correlation.ts) | x-correlation-id propagation |
| [apps/web-bff/src/middleware/logger.ts](apps/web-bff/src/middleware/logger.ts) | Structured JSON request logging |
...
| [apps/bills-api/src/data/bills.data.ts](apps/bills-api/src/data/bills.data.ts) | In-memory bill store; makeBill() always computes balance |
...
**Principle:** [A3](../suite-architecture-standards.md#a3) — Frontends Call the BFF. Only the BFF.
...
**Principle:** [E3](../suite-architecture-standards.md#e3) — Data at the Right Granularity
...
**Principle:** [E3](../suite-architecture-standards.md#e3) — BFF aggregation (one call returns bill + payments merged from two domain APIs)
...
**Principle:** [A8](../suite-architecture-standards.md#a8) — Observability Is Not Optional

**Code:** [apps/web-bff/src/middleware/correlation.ts](apps/web-bff/src/middleware/correlation.ts) — Generates or propagates `x-correlation-id` on every request. The same ID flows through BFF → bills-api → payments-api logs, enabling distributed tracing.
...
**Principle:** [A9](../suite-architecture-standards.md#a9) — Cross-MFE communication via CustomEvent (not direct import)
...
**Principle:** [E4](../suite-architecture-standards.md#e4) — Authentication Is Centralized
...
**Principle:** [A3](../suite-architecture-standards.md#a3) — Balance validation lives at the BFF, not the MFE and not payments-api
...
**Principle:** [A3](../suite-architecture-standards.md#a3) (BFF proxied), [E5](../suite-architecture-standards.md#e5) (structured response), cross-MFE event on success
...
**Principle:** [A9](../suite-architecture-standards.md#a9) — Shell refreshes its own state in response to a CustomEvent from the MFE; no full reload, no shared state object
...
**Principle:** [E5](../suite-architecture-standards.md#e5) — Fail Gracefully, Not Silently
...
**Principle:** [A5](../suite-architecture-standards.md#a5) and [P4](../principles.md#p4) — Independent Deployability
...
**Try it:** Open [apps/bills-api/src/data/bills.data.ts](apps/bills-api/src/data/bills.data.ts) and find the `makeBill()` function. Every bill's `balance` field is computed as `totalAmount - amountPaid`. It is never stored or hardcoded.

**Principle:** [E8](../suite-architecture-standards.md#e8) — Vocabulary Discipline

**Code:** [apps/bills-api/src/data/bills.data.ts](apps/bills-api/src/data/bills.data.ts) — `makeBill()` always computes `balance: totalAmount - amountPaid`. This ensures the balance is always consistent with the source of truth (total and paid amounts).
...
**Principle:** [A9](../suite-architecture-standards.md#a9) — Angular auto-scopes all styles to each component; unscoped styles would bleed into the shell and sibling MFEs
...
**Principle:** [A8](../suite-architecture-standards.md#a8) — Observability Is Not Optional; [E10](../suite-architecture-standards.md#e10) — Logs Are Event Streams

**Code:** [apps/web-bff/src/middleware/correlation.ts](apps/web-bff/src/middleware/correlation.ts) — Generates UUID if absent, propagates if present. [apps/web-bff/src/middleware/logger.ts](apps/web-bff/src/middleware/logger.ts) — Structured JSON logs to stdout, one line per request.
...
|---------|-----|---------------------|
| Real authentication | Demo uses hardcoded `Bearer demo-token` | [E4](../suite-architecture-standards.md#e4) — centralized auth; replace with OAuth2/OIDC in production |
| Persistence layer | Domain APIs use in-memory data stores | [E2](../suite-architecture-standards.md#e2) — domain APIs would use PostgreSQL, MongoDB, etc. |
| Payment gateway integration | No Stripe/Braintree/Adyen | [A7](../suite-architecture-standards.md#a7) — vendor SDKs belong in domain APIs, not the BFF |
| Error boundaries | No UI fallback for MFE load failures | [A9](../suite-architecture-standards.md#a9) — production shells need error boundaries per remote |
| CI/CD pipelines | No GitHub Actions / CircleCI config | [P4](../principles.md#p4) — each app needs independent pipeline |
| API rate limiting | No throttling or quota enforcement | [E12](../suite-architecture-standards.md#e12) — production BFFs need rate limiting middleware |
| Real-time updates | No WebSocket / Server-Sent Events | [E11](../suite-architecture-standards.md#e11) — real-time events require event-driven architecture |
| Multi-tenancy | Single account only | [E8](../suite-architecture-standards.md#e8) — production requires tenant isolation |
| Internationalization | English only | [A9](../suite-architecture-standards.md#a9) — i18n belongs in MFEs, not BFF responses |
| OpenAPI specs | Not yet generated | [E1](../suite-architecture-standards.md#e1) — contract-first means OpenAPI before implementation |
...
- **[NAVIGATION-FLOWS.md](NAVIGATION-FLOWS.md)** — Mermaid sequence diagrams for every user interaction: bootstrap, bills list, bill detail aggregation, Pay Now CustomEvent, payment submission with balance guard, confirmation badge refresh, and a complete navigation map
- **[PARTNER-BFF-A2-DEMO.md](PARTNER-BFF-A2-DEMO.md)** — Side-by-side curl comparisons showing A2 (BFF Per Surface, Not Per Service) in action: web-bff vs partner-bff calling the same domain APIs with different auth, response shapes, and payment patterns

### Workspace Documentation

- [suite-architecture-standards.md](../suite-architecture-standards.md) — Full architectural principles (A1–A9, E1–E12)
- [sample-app-prompts.md](../sample-app-prompts.md) — The 14 build prompts used to construct this app
- [api-vs-headless-commerce.md](../api-vs-headless-commerce.md) — Platform vocabulary and API-first evaluation guide
- [principles.md](../principles.md) — Engineering and architectural principles (P1–P5)
- [definitions.md](../definitions.md) — Plain-English definitions for every term used in the standards
