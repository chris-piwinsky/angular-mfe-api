...
const PAYMENTS_API_URL =
  process.env.PAYMENTS_API_URL ?? 'http://localhost:4002';

export async function fetchPaymentsForBill(
  billId: string,
  correlationId: string,
): Promise<Payment[]> {
  const res = await fetch(
    `${PAYMENTS_API_URL}/v1/payments?billId=${encodeURIComponent(billId)}`,
    {
