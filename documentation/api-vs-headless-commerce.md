...
Before evaluating any platform, ask: *Can every operation — reads, writes, reconciliation, configuration, admin tasks — be performed via a documented API without touching a UI?*
...
When a vendor claims API-first billing, ask: *"Can you show me three integrations where the consuming team never needed to use the vendor's UI or contact vendor support to operate the system day-to-day?"* Retrofitted platforms cannot produce these references.
